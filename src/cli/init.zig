...
pub const Project = struct {
    ...
    templates_dir: []const u8 = "../../res/ptmpl/",
    ...
    pub fn createBaseFiles(self: Project) anyerror!void {
        const defaultp = "../../res/new";
        const filenames = .{ "Idle.toml", ".gitignore", "README.md", "build.is" }; //Plus out dir
        var files: [4]std.fs.File = .{};
        for (base_files) |fname, i| {
            const tmpl = @embedFile(path.join(self.allocator, defaultp, files[0]));
            files[i] = try di.createFile("{s}", .{fname});
            files[i].writeAll(tmpl);
        }
    ...
    ///
    pub fn createSrcFiles(self: Project, dir: Dir) anyerror!void {
        try dir.makeDir("src");
        const src_dir = try dir.openDir("src");
        var src_files = self.project_type.srcFiles();
        var files: [src_files.len]std.fs.File = .{};
        for (src_files) |fname, i| {
            const tmpl_path = path.join(self.allocator, defaultp, files[0]);
            const templ = @embedFile(tmpl_path);
            files[i] = try dir.createFile("{s}", .{fname});
            try files[i].writeAll(templ);
        }
    }
